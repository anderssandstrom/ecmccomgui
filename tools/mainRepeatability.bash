...
#   RESOLVERVALSNEG+="$RESOLVERVALNEG "
#   echo "Resolver value neg=$RESOLVERVALNEG"
#   DIFF=$(awk "BEGIN {print ($RESOLVERVALPOS-($RESOLVERVALNEG))}")
#   DIFFS+="$DIFF "
#   printf "%d | %.${DEC}f | %.${DEC}f | %.${DEC}f\n" $COUNTER $RESOLVERVALPOS $RESOLVERVALNEG $DIFF >> $REPORT
#done
#
## Opto
#bash ecmcReport.bash $REPORT ""
...
#   OPTOVALSNEG+="$OPTOVALNEG "
#   echo "Opto value neg=$OPTOVALNEG"
#   DIFF=$(awk "BEGIN {print ($OPTOVALPOS-($OPTOVALNEG))}")
#   DIFFS+="$DIFF "
#   printf "%d | %.${DEC}f | %.${DEC}f | %.${DEC}f\n" $COUNTER $OPTOVALPOS $OPTOVALNEG $DIFF >> $REPORT
#done
#
############### Pos 55 POS
#TRIGGVAL=3501
